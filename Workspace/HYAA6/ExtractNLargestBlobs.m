...
function cell = ExtractNLargestBlobs(cell, numberToExtract)
try
	% Get all the blob properties.  Can only pass in originalImage in version R2008a and later.
	[labeledImage, numberOfBlobs] = bwlabel(cell);
	blobMeasurements = regionprops(labeledImage, 'area'); % to get blobs areas 
	% Get all the areas
	allAreas = [blobMeasurements.Area];    %% get all number of areas for each blobs
