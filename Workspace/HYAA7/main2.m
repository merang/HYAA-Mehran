...
% properties = regionprops(OriginalImage, {'Area','Centroid', 'Orientation', 'Perimeter'});
% properties = struct2table(properties);
%%%%%%%%%%%%%%%%%%%%%%% Great blank  traps image %%%%%%%%%%%%%%%%%%%%%%%%%
% BW_out = bwpropfilt(BW_in, 'Area', [-Inf, X - eps(X)]);
% BW_out = bwpropfilt(OriginalImage, 'Area', [82,200]);% fillter object area

% figure ();
% imshow(BW_out)
% title('Blank Trap')
%%%%%%%%%%%%%%%%%%%%%%%%%%%%% Cells Locations %%%%%%%%%%%%%%%%%%%%%%%%%%%
%  cell=gsubtract(originalImage,BW_out);
 cell = imclearborder(originalImage);  % Get rid of border of image 
...
%         ref=BW_out(top:bottom,left:right); 
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        %%%%%%%%%%%%%%%%%%%%%%%%% Partitioned Image %%%%%%%%%%%%%%%%%%%%%%%
        cropImage=originalImage(top:bottom,left:right);
        ...
        ref = imadjust(reference,[0.4 0.69],[]);
        refBW= im2bw(ref);
        refBW=imcomplement(refBW);
        REFBW=imfill(refBW,'holes');
        tformEstimate = imregcorr(cropImage,ref);
...
%       msgbox('Done with demo!');
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        subplot(4,4,13)
        [~,moving]=imregdemons(cell,refBW,[500 400 200],'AccumulatedFieldSmoothing',1.3);
        imshow(moving);
        
        subplot(4,4,14)
        imshowpair(movingReg,cell);
         hold on
        visboundaries(movingReg,'Color','r');
         hold off
         title('Original Cell(pink) & displacement cells(green)'); 
...
%       registered=imregister(cropImage,REFBW,'translation',optimizer,metric);
%       figure()
%       imshowpair(registered,REFBW);
%       title('first test');
%  
%       registered=imregister(cropImage,REFBW,'similarity',optimizer,metric);
%       figure()
%       imshowpair(registered,REFBW);
%       title('Final test');
